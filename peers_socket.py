...
class PeerSocket:
    ...
    def tit_for_tat(self):
        ...
        if not self.unchoked_peers: # Caso nenhum peer tenha o bloco
            self.unchoked_peers = {TRACKER_PORT}
    ...
    def request_block_from_peer(self, target_pid, block_idx):
        ...
        print(f"[{self.peer_id}] Enviando pedido do bloco {block_idx} para {peer_id} (porta {port})")
        msg = {
    ...
    def run(self):
        ...
        while not all(self.blocks_owned):
            ...
            #Se nenhum peer tem o bloco, pede ao tracker (seed)
            for block_idx in range(TOTAL_FILE_BLOCKS):
                # Verifica se NENHUM peer (exceto tracker) tem o bloco
                block_is_rare = all(
                    not blocks[block_idx]
                    for pid, blocks in self.peer_blocks.items()
                )

                print(f"Bloco {block_idx} raro? {block_is_rare}, Tracker tem bloco? Sim")

                if block_is_rare:
                    print(f"[{self.peer_id}] Ninguém tem o bloco {block_idx}. Pedindo ao Tracker.")
